...
Así, las ecuaciones \@ref(eq:ETrend) y \@ref(eq:VarTrend) significan que el proceso descrito por la ecuación \@ref(eq:YTrend) es estacionario en varianza, aunque no en media. De esta forma, a partir de este momento diremos que una serie será estacionaria alrededor de una tendencia determinística si cumple con las condiciones establecidas en las ecuaciones  \@ref(eq:YTrend), \@ref(eq:ETrend) y \@ref(eq:VarTrend).
...
1. **(Teórico)** Considere una caminata aleatoria con deriva: $X_t = \delta + X_{t-1} + U_t$, con condición inicial $X_0$ conocida y $U_t$ un proceso puramente aleatorio con media cero y varianza $\sigma^2$.

    a. Mediante sustitución recursiva, demuestre que $X_t = X_0 + \delta t + \sum_{i=1}^{t} U_i$.
    b. Calcule $\mathbb{E}[X_t]$ y $Var[X_t]$ y explique por qué el proceso viola la definición de estacionariedad débil.
    ...
    d. Explique por qué la primera diferencia $\Delta X_t$ sí es estacionaria y qué significa, en consecuencia, que $X_t$ sea un proceso $I(1)$.
...
3. **(Teórico)** Las pruebas ADF y KPSS plantean hipótesis nulas opuestas, por lo que suelen aplicarse de forma conjunta. Construya una tabla de $2 \times 2$ con los cuatro resultados posibles de aplicar ambas pruebas a una misma serie (rechazar / no rechazar en cada prueba) y, para cada celda:
...
5. **(Computacional)** Utilice la serie del IGAE contenida en `BD/Datos_Ad.RData` (columna `IGAE_Ad`, serie desestacionalizada) y construya su logaritmo, $LIGAE_t$. Determine el orden de integración de la serie siguiendo la guía de decisión de este capítulo:
