...
```{r fig74, warning=FALSE, fig.cap="Comparación de Series en Diferencias Anuales", fig.align='center'}
...
```{r fig75, warning=FALSE, fig.cap="Comparación de Series en Diferencias", fig.align='center'}
...
summary(df_nA); summary(df_nB); summary(df_nC)
summary(df_dA); summary(df_dB); summary(df_dC)
...
knitr::kable(
  rbind(urdf_fila("$LTC_t$ — Modelo A (tendencia)",        df_nA),
        urdf_fila("$LTC_t$ — Modelo B (intercepto)",       df_nB),
        urdf_fila("$LTC_t$ — Modelo C (sin det.)",         df_nC),
        urdf_fila("$\\Delta LTC_t$ — Modelo A (tendencia)",  df_dA),
        urdf_fila("$\\Delta LTC_t$ — Modelo B (intercepto)", df_dB),
        urdf_fila("$\\Delta LTC_t$ — Modelo C (sin det.)",   df_dC)),
  col.names = c("Modelo", "Estadístico $t$", "CV 1%", "CV 5%", "CV 10%"),
...
summary(adf_nA); summary(adf_nB); summary(adf_nC)
summary(adf_dA); summary(adf_dB); summary(adf_dC)

knitr::kable(
  rbind(urdf_fila("$LTC_t$ — Modelo A (tendencia)",        adf_nA),
        urdf_fila("$LTC_t$ — Modelo B (intercepto)",       adf_nB),
        urdf_fila("$LTC_t$ — Modelo C (sin det.)",         adf_nC),
        urdf_fila("$\\Delta LTC_t$ — Modelo A (tendencia)",  adf_dA),
        urdf_fila("$\\Delta LTC_t$ — Modelo B (intercepto)", adf_dB),
        urdf_fila("$\\Delta LTC_t$ — Modelo C (sin det.)",   adf_dC)),
  col.names = c("Modelo", "Estadístico $t$", "CV 1%", "CV 5%", "CV 10%"),
...
summary(pp_nA); summary(pp_nB)
summary(pp_dA); summary(pp_dB)
...
knitr::kable(
  rbind(urpp_fila("$LTC_t$ — Modelo A (tendencia)",          pp_nA),
        urpp_fila("$LTC_t$ — Modelo B (intercepto)",         pp_nB),
        urpp_fila("$\\Delta LTC_t$ — Modelo A (tendencia)",  pp_dA),
        urpp_fila("$\\Delta LTC_t$ — Modelo B (intercepto)", pp_dB)),
  col.names = c("Modelo", "Estadístico $Z_\\tau$", "CV 1%", "CV 5%", "CV 10%"),
...
summary(kpss_nA); summary(kpss_nB)
summary(kpss_dA); summary(kpss_dB)
...
knitr::kable(
  rbind(urkpss_fila("$LTC_t$ — Modelo A ($\\tau$, tendencia)",         kpss_nA),
        urkpss_fila("$LTC_t$ — Modelo B ($\\mu$, intercepto)",         kpss_nB),
        urkpss_fila("$\\Delta LTC_t$ — Modelo A ($\\tau$, tendencia)", kpss_dA),
        urkpss_fila("$\\Delta LTC_t$ — Modelo B ($\\mu$, intercepto)", kpss_dB)),
  col.names = c("Modelo", "Estadístico", "CV 1%", "CV 5%", "CV 10%"),
...
za_df <- data.frame(
  Modelo      = c("Modelo A (intercepto)", "Modelo B (tendencia)", "Modelo C (ambos)"),
...
knitr::kable(
  za_df,
  col.names = c("Modelo", "Estadístico", "CV 1%", "CV 5%", "CV 10%", "Quiebre est."),
