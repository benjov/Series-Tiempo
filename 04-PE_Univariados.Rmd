...
Observemos que la ecuación (\ref{EDO_S_LP}) se puede reescribir si consideramos la formulación que en la literatura se denomina como la descomposición de Wold, en la cual se define que es posible asumir que $\psi_j = a_1^j$ y se considera el caso en el cual $ |a_1| < 1 $, de esta forma tendremos que por ejemplo cuando:
\begin{equation*}
...
Donde $\tau = 1, 2, 3, \ldots$ y $ |a_1| < 1$. Este último resultado significa que cuando el proceso autoregresivo es de orden 1 (es decir, AR(1)) la función de autocorrelación teóricamente es igual al parámetro $a_1$ elevado al número de rezagos considerados. No obstante, note que esto no significa que la autocorrelación observada sea como lo expresa en planteamiento anterior. Por el contrario, una observación sencilla mostraría que la autocorrelación observada sería ligeramente distinta a la autocorrelación teórica.
...
```{r fig41, fig.cap = "Comportamiento del Proceso Real ('Estimado')", fig.align='center'}
...
```{r fig42, fig.cap = "Comportamiento del Proceso Teórico", fig.align='center'}
...
```{r fig43, fig.cap = "Funcion de Autocorrelacion Real", fig.align='center'}
...
```{r fig44, fig.cap = "Funcion de Autocorrelacion Teórica", fig.align='center'}
...
```{r fig45, fig.cap = "Comportamiento de los Procesos Real y Teórico", fig.align='center'}
...
Así, solo si el polinomio anterior tiene raíces cuyo valor absoluto sea menor a uno ($ |\lambda_i| < 1$) y si $1 - a_1 L  - a_2 L^2 - a_3 L^3  - \ldots - a_p L^p < 1$ podremos decir que el proceso es convergente y estable. Lo anterior significa que la ecuación (\ref{Pol_A}) puede expresarse en términos de la descomposición de Wold o como la suma infinita de términos como:
\begin{equation}
...
Datos <- read_excel("BD/Base_Transporte_ARIMA.xlsx", sheet = "Datos", col_names = TRUE)
...
En la ecuación (\ref{ARMA11_Sol}) las condiciones de estabilidad y de invertibilidad del sistema (de un MA a un AR, y viceversa) estarán dadas por: $ |a_1| < 1$ y $ |b_1| < 1$. Adicionalmente, la ecuación (\ref{ARMA11_Sol}) expresa cómo una serie que tiene un comportamiento $ARMA(1, 1)$ es equivalente a una serie modelada bajo un $MA(\infty)$.
...
```{r fig412, fig.cap = "Pronóstico de la serie $PaxNAl_t$ a partir de una ARMA(4,6) en diferencias logaritmicas", fig.align='center'}
...
Predict_Datos <- read_excel("BD/Predict_Base_Transporte_ARIMA.xlsx", sheet = "Datos", col_names = TRUE)
...
Pronostico_Arima <- read_excel("BD/Pronostico_Arima.xlsx", sheet = "Datos", col_names = TRUE)
