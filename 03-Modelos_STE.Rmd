...
obstante, esto no resulta ser práctico cuando se opere más adelante en
el curso. Por ello, en el curso y, en general, en casi todos los textos
lo hacen, sólo nos enfocaremos en sus primero y segundo momentos. Es
decir, en sus media o valor esperado: \begin{equation*}
    \mathbb{E}[X_t]
\end{equation*}
...
$T \rightarrow \infty$ sus correspondientes momentos muestrales, tienden
a los verdaderos valores poblacionales, los cuales definiremos como
$\mu$, para la media, y $\sigma^2_X$ para la varianza.
...
Estas condiciones se les conoce como *propiedades de consistencia* para
las variables aleatorias. Sin embargo, éstas no pueden ser probadas. Por
ello se les denomina como un supuesto que pueden cumplir algunas de las
series. Más importante aún: **un proceso estocástico que tiende a estar
en equilibrio estadístico en un orden ergódico, es estacionario**.
...
ser reescrito como: \begin{equation}
    X_t = \sum^t_{j = 0} U_j
    (\#eq:em2)
\end{equation}
...
Así, el proceso estocástico dado por la caminata aleatoria sin un
...
sabemos que esta se mantiene asintóticamente. Ljung y Box
[-@ljungbox1978] propusieron la siguiente modificación de la estadística
para muestras pequeñas: \begin{equation}
...
[-@godfrey1978]. La
cual, al igual que las estadísticas $Q$ y $Q^*$, la hipótesis nula está
dada por:
...
son una serie de tiempo en el período que va de enero de 2000 a mayo de
2023, es decir, 281 observaciones. Como se puede apreciar en la Figura
...
[^03-modelos_ste-1]: Los datos y algoritmo está disponible en el
    repositorio de GitHub y corresponde a la Clase 3.
...
se puede rechazar la hipótesis nula de que $H_0: \hat{\rho}(p) = 0$,
...
: (#tab:foo) Estadísticas descriptivas del número de pasajeros en el
Metro de la CDMX, enero de 2000 a junio de 2019
...
diferencia estudiadas al inicio del curso: $|a_1| < 1$, por lo que
...
De esta forma tendremos que, por ejemplo, cuando: \begin{equation*}
...
para el proceso puramente aleatorio $U_t$ su varianza y covarianza puede
verse como $\mathbb{E}[U_t, U_s] = \sigma^2$, para $t = s$, y
...
expresa en el planteamiento anterior. Por el contrario, una observación
sencilla mostraría que la autocorrelación observada sería ligeramente
distinta a la autocorrelación teórica. Ahora veamos algunos ejemplos.
...
del proceso AR(2) en la ecuación \@ref(eq:AR2Eq) será dada por una
expresión como sigue: \begin{equation}
...
de estabilidad de la solución y cómo una condición de invertibilidad.
...
como un proceso $AR(q)$ sobre la serie $U_t$.
...
El cual debe cumplir que $|x_i| < 1$ y que
...
Akaike (AIC). Finalmente, podemos determinar si la solución serán
...
MA, es decir, cómo un $MA(q)$ de la serie $X_t$ puede ser reexpresada
como un $AR(\infty)$ de la serie $U_t$, y viceversa un $AR(p)$ de la
serie $X_t$ puede ser reexpresada como un $MA(\infty)$.
...
el segundo es las veces que se tuvo que diferenciar la serie analizada,
...
Está ecuación es equivalente a la expresión: \begin{eqnarray}
...
invertibilidad son que las raíces de los polinomios característicos
...
| AR(p)  |    No hay rompimientos     |     Rompimientos en la función     |
...
no requiere el mismo supuesto. No obstante, para el curso utilizaremos
...
Así, el orden se elige de acuerdo a aquel criterio de información que
...
Por esta razón, durante el curso solo utilizaremos el criterio de Akaike
...
del análisis concluimos que el modelo más adecuado es el 24, el cual
considera un $ARMA(4, 6)$, con variables dummies para controlar la
estacionalidad de los meses de enero, febrero, julio y diciembre. Más
adelante mostramos los resultados del modelo.
...
un total de 36 modelos ARMA y determinamos que el orden que minimiza el
criterio de Akaike es un $ARMA(4, 6)$. El siguiente cuadro muestra los
resultados para este nuevo modelo. No lo mostramos en esta sección, pero
ambos modelos reportados tienen raíces de sus respectivos polinomios
característicos menores a 1 en valor absoluto. En la Figura
\@ref(fig:fig416) mostramos los residuales ahora ajustados por las
dummies de mayo y junio de 2009.
...
mod1 <- arima(DLPax_Nal, order = c(4, 0, 6),
              xreg = xreg_base, method = "ML")
mod2 <- arima(DLPax_Nal, order = c(4, 0, 6),
              xreg = cbind(xreg_base, D_May2009, D_Jun2009), method = "ML")

lbl_map <- c(ar1="$DLPaxNal_{t-1}$", ar2="$DLPaxNal_{t-2}$",
             ar3="$DLPaxNal_{t-3}$", ar4="$DLPaxNal_{t-4}$",
             ma1="$\\hat{U}_{t-1}$",  ma2="$\\hat{U}_{t-2}$",
             ma3="$\\hat{U}_{t-3}$",  ma4="$\\hat{U}_{t-4}$",
             ma5="$\\hat{U}_{t-5}$",  ma6="$\\hat{U}_{t-6}$",
...
un conjunto de dummies. No obstante, en el caso particular que
...
general, la representación es de una serie no estacionaria, aunque si
$D = d = 0$ y las raíces del polinomio característico (de los polinomios
del lado izquierdo de la ecuación \@ref(eq:ARIMAseas) todas son más
grandes que 1 en valor absoluto, el proceso modelado será estacionario.
...
trimestrales de algunas series como el Gross Domestic Product (GNP, por
sus siglas en Inglés), los agregados monetarios M1, empleo, etc., de los
...
En la ecuación \@ref(eq:HPEq) se asume que el ajuste de la ruta seguida
por $g_t$ es resultado de la suma de los cuadrados de su segunda
diferencia. En esa misma ecuación asumiremos que $c_t$ son las
...
de Hodrick y Prescott [-@hodrickprescott1997] implican resolver el siguiente problema
...
minimizar la serie $\{ \tau_t \}_{t=-1}^T$:
...
\end{equation}
```
Método modificado de HP consiste en minimizar los valores de la serie
$\{ \tau_t \}_{t=1}^T$: \begin{equation}
  \sum_{t=1}^T (y_t - \tau_t)^2 + \lambda \sum_{t=2}^{T-1} [(\tau_{t+1} - \tau_t) - (\tau_t     - \tau_{t-1})]^2 + \lambda_{ss} \sum_{t=T-j}^{T} [\Delta \tau_t - u_{ss}]
\end{equation}

Donde $\lambda$ es una parámetro fijo (determinado ex-ante), $\tau_t$ es
un componente de tendencia de $y_t$, y los nuevos parámetros son
$u_{ss}$ y $\lambda_{ss}$ ajustadas por el procedimiento de Marcet y
Ravn [-@marcetravn2004].

Este procedimiento asume que parte del filtro HP y que esta versión
